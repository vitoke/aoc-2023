...
for (const game of games) {
  const [idText, allBags] = game.split(":");
  const id = parseInt(idText.match(/\d+/g)[0]);
  total += id;
  const bags = allBags.split(";");
  const allBagsPossible = bags.every((bag) => {
    const colors = bag.split(",");
    const red = parseInt(
      colors.find((c) => c.includes("red"))?.match(/\d+/g)[0] || 0
    );
    if (red > 12) return false;
    const green = parseInt(
      colors.find((c) => c.includes("green"))?.match(/\d+/g)[0] || 0
    );
    if (green > 13) return false;
    const blue = parseInt(
      colors.find((c) => c.includes("blue"))?.match(/\d+/g)[0] || 0
    );
    if (blue > 14) return false;
    return true;
  });
  if (allBagsPossible) answer += id;
}

console.log("total of all game ids:", total);
console.log("total of all possible game ids:", answer);
